...
def detect_image():
    ...
    log_detection(f"Ảnh: {os.path.basename(file_path)} | Đối tượng: {len(results[0].boxes)}")
...
def process_stream():
    ...
    if frame_count % 5 == 0:  # skip frame để nhẹ CPU
        ...
        log_detection(f"{running_mode.upper()} | FPS: {fps:.2f} | Đối tượng: {len(results[0].boxes)} | Frame: {frame_count}")
